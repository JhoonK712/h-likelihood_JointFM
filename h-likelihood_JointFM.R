...
###------------------------------------- Case 1 - Bivariate survival data -------------------------------------###
data(kidney)
dataset <- kidney
...
###------------------------------------- Case 2 - Semi-competing risks survival data -------------------------------------###
data(dataOvarian1)
data(dataOvarian2)
...
###------------------------------------- Case 3 - Competing risks survival data - bladder data -------------------------------------###
data(bladder)
center_count <- as.data.frame(table(bladder$center))
names(center_count) <- c("center", "center_n")

bladder2 <- merge(bladder, center_count, by.x = "center", by.y = "center", all.x = TRUE)
bladder_sorted <- bladder2[order(bladder2$center_n), ]
head(bladder_sorted)

names(bladder_sorted)[names(bladder_sorted) == "surtime"] <- "time"
bladder_sorted$id <- as.numeric(factor(bladder_sorted$center, levels = unique(bladder_sorted$center)))

dataset <- bladder_sorted
attach(dataset)

P <- dim(dataset)[1]
EVENT <-matrix(0, P, 4)

EVENT[,1] <- as.matrix(dataset$time, P, 1) # t_event1
EVENT[,2] <- as.matrix(dataset$time, P, 1) # t_event2
EVENT[,3] <- as.matrix(ifelse(dataset$status==1, dataset$status, 0), P, 1) # event1
EVENT[,4] <- as.matrix(ifelse(dataset$status==2, 1, 0), P, 1) # event2
...
formula <-  ~ CHEMO + AGE + (1|id)

X <- model.matrix(formula)
X <- data.frame(X)
X <- as.matrix(X)
Z <- as.matrix(X[, c(2:(dim(X)[2]-1))])
...
comp_random_figure <- ggplot(result, aes(x = x, y = v_h) ) +
		geom_point(size = 1.5) +
		geom_errorbar(aes(ymin = LB, ymax = UB), width = 0.2) +
		geom_hline(yintercept = 0) +
		labs(x = "Centre number", y = "Estimated center effects") +
		theme_minimal()+
		theme(
			panel.grid.major = element_blank(),
			panel.grid.minor = element_blank(),
...
# Plot
esti_g1 <- Ms%*%t(res_g1)	# cmprskCox.reg()
esti_g2 <- Ms%*%t(res_g2)	# cmprskCox.reg()
...
comp_base_figure <- ggplot(plot_data, aes(x = Time, y = Hazard, color = Group)) +
	geom_line(size = 1) +
	scale_color_manual(values = c("Recurrence" = "black", "Death" = "red")) +
